...
// 使用 config.js 中的文章 API 配置
const ARTICLES_API_CONFIG = CONFIG.ARTICLES;
...
async function fetchArticlesData() {
  try {
    ...
    const response = await fetch(ARTICLES_API_CONFIG.URL, {
      ...
      body: ARTICLES_API_CONFIG.Body
        ? JSON.stringify(ARTICLES_API_CONFIG.Body)
        : undefined,
    });
    ...
    // 检查 URL 是否为 RSS XML 格式
    const isRSS =
      ARTICLES_API_CONFIG.URL.includes(".xml") ||
      ARTICLES_API_CONFIG.URL.includes("atom");
    let articles = [];
    ...
    if (isRSS) {
      ...
      // 如果没有 entry 元素，尝试处理 RSS 2.0 格式
      if (entries.length === 0) {
        entries = xml.querySelectorAll("item");
      }

      articles = Array.from(entries).map((entry) => {
        // 检查是 Atom 还是 RSS 2.0 格式
        const isAtom = entry.tagName === "entry";

        return {
          id:
            entry.querySelector(isAtom ? "id" : "guid")?.textContent ||
            entry.querySelector("link")?.textContent ||
            "",
          title: entry.querySelector("title")?.textContent || "",
          content:
            entry.querySelector(isAtom ? "content" : "description")
              ?.textContent ||
            entry.querySelector("summary")?.textContent ||
            "",
          created_at:
            entry.querySelector(isAtom ? "updated" : "pubDate")?.textContent ||
            new Date().toISOString(),
          link: isAtom
            ? entry.querySelector("link")?.getAttribute("href")
            : entry.querySelector("link")?.textContent || "",
          author:
            entry.querySelector(isAtom ? "author name" : "author")
              ?.textContent || "",
        };
      });
    } else {
...
async function fetchFriendsData() {
  try {
    const response = await fetch("https://friends-api.050815.xyz/friends");
    if (!response.ok) {
            ...
            <div class="friends-container">
                <div class="friends-grid">
                    ${friendsData
                      .map(
                        (friend) => `
                        <a href="${friend.url}" class="friend-card ${friend.recommended ? "friend-card-recommended" : ""}" target="_blank" rel="noopener">
                            ${friend.recommended ? '<span class="friend-recommended-badge">推荐</span>' : ""}
                ...
                <div class="site-info">
                    ...
                    <div class="site-info-content">
                        <div class="site-info-item">
                            ...
                            <span class="site-info-value"><a href="javascript:void(0);" onclick="copyToClipboard('塔罗会')">塔罗会</a></span>
                        </div>
                        <div class="site-info-item">
                            <span class="site-info-label">地址：</span>
                            <span class="site-info-value"><a href="javascript:void(0);" onclick="copyToClipboard('https://cv.050815.xyz/')">https://cv.050815.xyz/</a></span>
                        </div>
                        <div class="site-info-item">
                            <span class="site-info-label">图标：</span>
                            <span class="site-info-value"><a href="javascript:void(0);" onclick="downloadImage('https://cdn.jsdmirror.com/gh/zsxcoder/github-img@main/img/yxk-avatar.avif', 'yxk.avif')">点击下载</a></span>
                        </div>
                        <div class="site-info-item">
                            <span class="site-info-label">简介：</span>
                            <span class="site-info-value"><a href="javascript:void(0);" onclick="copyToClipboard('每一段旅行，都有终点。')">每一段旅行，都有终点。</a></span>
                        </div>
                        <div class="site-info-item">
                            <span class="site-info-label">申请友链：</span>
                            <span class="site-info-value"><a href="https://github.com/yxksw/Friends/issues/new?template=friend-link-request.yml">议题 申请</a></span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    try {
      ...
      if (staticResponse.ok) {
        ...
        const repoDetails = staticData.repos.map((repo) => ({
          ...
          last_commit:
            repo.lastCommit?.commit?.author?.date || repo.last_commit,
        ...
        const filteredRepos = repoDetails.filter(
          (repo) => repo.name !== "yuazhi" && repo.name !== "yxksw/yuazhi",
        );
      ...
      console.log(
        "Pre-fetched data not available, falling back to API:",
        staticError.message,
      );
    }
...
async function fetchContributionData(
  ...
  try {
    ...
    if (staticData) {
      ...
      console.log(
        `Using static data: ${allEvents.length} events, ${userRepos.length} repos, ${allStars.length} stars, ${allForks.length} forks`,
      );
    } else {
      // 调用 API 获取数据
      ...
      console.log(
        `Fetched from API: ${allEvents.length} events, ${userRepos.length} repos, ${allStars.length} stars, ${allForks.length} forks`,
      );
    }
                                ...
                                <div class="activity-content">
                                    ...
                                    `
                                        ...
                                        <div class="commit-messages commit-bg-red" data-activity-id="${activity.id || Math.random().toString(36).substr(2, 9)}">
                                            <div class="commit-message-item">
                                                <span class="commit-message-text">${activity.count > 0 ? `Pushed ${activity.count} commit${activity.count > 1 ? "s" : ""} to ${activity.branch || "main"}` : `Pushed to ${activity.branch || "main"}`}</span>
                                            </div>
                                        </div>
                                    `
                                    }
    ...
    content.innerHTML = `
            ...
            <div class="github-stats-section">
                ...
                <div class="stats-container">
                    <img src="https://github.chenc.dev/https://raw.githubusercontent.com/yxksw/contribution/main/metrics.isocalendar.svg" 
                         alt="GitHub Contributions Calendar" 
                         class="github-stats-image"
                         loading="lazy">
                    <img src="https://github.chenc.dev/https://raw.githubusercontent.com/yxksw/contribution/main/metrics.base.svg" 
...
function showTelegram(event) {
  ...
  const telegramUrl =
    "https://cdn.jsdmirror.com/gh/zsxcoder/github-img@main/img/telegram-invite.avif";
  showQRCode(
    "Telegram",
    telegramUrl,
    "直接点击此处加好友",
...
function showQQ(event) {
  ...
  const qrCodeUrl = isDarkMode
    ? "https://cdn.jsdmirror.com/gh/zsxcoder/github-img@main/img/qq-dark.avif"
    : "https://cdn.jsdmirror.com/gh/zsxcoder/github-img@main/img/qq-invite.avif";
  showQRCode(
    "QQ",
    qrCodeUrl,
    "直接点击此处加好友",
    "https://qm.qq.com/q/rMcURttFFC",
  );
}
      ...
      if (staticResponse.ok) {
        ...
        if (staticData.stars && staticData.stars.length > 0) {
          ...
          return staticData.stars.map((star) => ({
            ...
            full_name:
              star.full_name ||
              `${star.owner?.login || "unknown"}/${star.name}`,
            owner: {
...
function createMemoHTML(memo) {
  ...
  if (reactions && reactions.length > 0) {
    reactions.forEach((reaction) => {
      ...
      groupedReactions[reactionType] =
        (groupedReactions[reactionType] || 0) + 1;
    });
  }
  ...
  const resourcesHTML =
    resources.length > 0
      ? `<div class="memo-resources">${resources
          .map((resource) => {
            // 新API可能有不同的资源结构
            const imageUrl =
              resource.externalLink || resource.url || resource.link;
            if (imageUrl) {
...
async function renderMemos() {
  try {
    ...
    // 加载更多按钮（当还有更多说说时显示）
    const hasMoreMemos =
      currentDisplayCount < allUnpinnedMemos.length || hasMorePages;
...
async function loadMoreMemos() {
  ...
  try {
    ...
    if (remainingLocalCount > 0) {
      ...
      const nextBatchMemos = allUnpinnedMemos.slice(
        nextBatchStart,
        nextBatchEnd,
      );
...
function initLightbox() {
  ...
  const allImages = document.querySelectorAll(
    ".memo-resources img, .readme-content img",
  );
...
async function showArticleDetail(article) {
  ...
  let articleContent = article.text || article.content;
  ...
  if (articleContent) {
    ...
    try {
      // 确保内容是字符串
      const contentStr = String(articleContent);
      // 检查 marked 是否可用
      if (typeof marked === "undefined" || !marked.parse) {
        console.error("marked is not available:", typeof marked);
        htmlContent =
          "<p>Markdown解析器未加载</p><pre>" +
          contentStr.replace(/</g, "&lt;").replace(/>/g, "&gt;") +
          "</pre>";
      } else {
        htmlContent = marked.parse(contentStr);
      }
    } catch (e) {
      console.error("marked.parse error:", e);
      htmlContent = "<p>Markdown解析错误: " + e.message + "</p>";
    }
    articleModal.querySelector(".article-modal-body").innerHTML = htmlContent;
    hljs.highlightAll();
  } else {
